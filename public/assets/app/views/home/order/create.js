...
App.views.define(() => {
  const render = pars => {
    ...
    const confirmAddComponent = els.confirmAddComponent.kendoDialog({
      title: 'How many?',
      modal: true,
      actions: [{
        primary: true,
        ...
        action: e => {
          details.insertItem({
            quantity: addComponentModel.quantity,
            componentId: addComponentModel.componentId,
            orderId: model.order.id
          })
          return true
        }
    ...
    const drop = els.detail.kendoDropTarget({
      ...
      drop: e => {
        ...
        addComponentModel.set('componentId', item.id)
    ...
    const show = m => {
      ...
      $.when($.ajax({
        ...
        .then(data => {
          ...
          $.when(details.loadData(), components.loadData()).then(resizeSplit)
          mainFade.play()
        })
    }
